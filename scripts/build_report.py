...
body(
    ...
    "changes, regulatory news—from Bloomberg, Reuters, and FinHub. The Fundamentals "
...
body(
    "Challenge 2 — News API ordering bug. The News Analyst fetches articles from the "
    "Yahoo Finance endpoint and then applies a date filter. Yahoo Finance returns "
    "articles in reverse-chronological order and terminates after a fixed record count; "
    "when called in 2026 with a January 2024 target window, the response contains only "
    "2025–26 articles, all of which are discarded by the filter, leaving the agent with "
    "empty input. The correct implementation applies the date constraint as a query "
    "parameter before fetching—but Yahoo Finance's public endpoint does not support "
    "date-bounded queries natively. We replaced the live feed with a pre-filtered static "
    "dataset from the GDELT Project, extracting AMZN-relevant articles from December "
    "2023 through January 2024 and formatting them to match the schema expected by the "
    "News Analyst agent. The Social Media Analyst's dependency on a locally cached "
    "Reddit dataset—not released with the codebase—was replaced with a Kaggle dataset "
    "of r/wallstreetbets and r/stocks submissions filtered to AMZN mentions over the "
    "same period."
)
body(
