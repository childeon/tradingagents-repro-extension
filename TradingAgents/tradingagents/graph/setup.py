...
# Changes: Added ablation mode wiring: skips debate layer or risk layer based on ablation_mode config.
...
class GraphSetup:
    ...
    def setup_graph(
        ...
        for i, analyst_type in enumerate(selected_analysts):
            ...
            # Connect to next analyst or downstream decision layer if this is the last analyst
            if i < len(selected_analysts) - 1:
                next_analyst = f"{selected_analysts[i+1].capitalize()} Analyst"
                workflow.add_edge(current_clear, next_analyst)
            elif self.ablation_mode == "no_research_debate":
        ...
        if self.ablation_mode == "no_risk_layer":
